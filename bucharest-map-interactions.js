(()=>{
  ...
  function enhance(){
    ...
    if(!container){currentMap?.remove();currentMap=null;return}
    ...
    centres.forEach((c,i)=>{
      ...
      wrap.className='map-centre-marker';
      if(c.name==='Spitalul Clinic de Urgență (Floreasca)')wrap.classList.add('floreasca-marker');
      button.className='bucharest-marker';
      button.type='button';
      button.innerHTML=`<span>${i+1}</span>`;
      button.setAttribute('aria-label',`${c.name}, ${c.address}`);
      wrap.append(button);
  ...
  window.addEventListener('fdbs:route-rendered',enhance);
  enhance();
})();
